...
namespace IO
{
    ...
    bool FITKAbstractHDF5Adaptor::readGeometryAdaptor(Core::FITKAbstractNDataObject* comm, H5::Group & h5Group, QString adaptorName)
    {
        //调用适配器读
        if (!comm) return false;
        auto adaptor = FITKIOADAPTORFACTORY->createT<FITKAbstractHDF5Adaptor>("HDF5", adaptorName);
        ...
        delete adaptor;
        if (!ok) _reader->consoleMessage(3, QString("Read %1 Error,File:%2 Line:%3").arg(adaptorName).arg(__FILE__).arg(__LINE__));
        return ok;
    }

    bool FITKAbstractHDF5Adaptor::writeGeometryAdaptor(Core::FITKAbstractNDataObject* comm, H5::Group & h5Group, QString adaptorName)
    {
        //调用适配器写
        if (!comm) return false;
        auto adaptor = FITKIOADAPTORFACTORY->createT<FITKAbstractHDF5Adaptor>("HDF5", adaptorName);
        ...
        delete adaptor;
        if (!ok) _writer->consoleMessage(3, QString("Write %1 Error,File:%2 Line:%3").arg(adaptorName).arg(__FILE__).arg(__LINE__));
        return ok;
    }
    ...
    int FITKAbstractHDF5Adaptor::readIntAttribute(H5::H5Object& h5group, std::string name, bool* b /*= nullptr */)
    {
        ...
        space.selectAll();
        int size = space.getSelectNpoints();
        if (size > 1)
        {
            int* data = new int[size];
            att.read(H5::PredType::NATIVE_INT, data);
            buf = data[0];
            delete[] data;
            return buf;
        }
    ...
    bool FITKAbstractHDF5Adaptor::readIntAttribute(H5::H5Object & h5group, std::string name, int* buff, int n, int m)
    {
        if (!h5group.attrExists(name)) return false;
    ...
    bool FITKAbstractHDF5Adaptor::readDoubleAttribute(H5::Group & h5group, std::string name, double * buff, int n, int m, bool * b)
    {
        if (!h5group.attrExists(name)) return false;
        H5::Attribute transAttr = h5group.openAttribute(name);
        hsize_t       dim[2] = { n, m };
    ...
    bool FITKAbstractHDF5Adaptor::readCharDataSet(H5::H5Object & h5group, std::string name, char *& data, int len)
    {
        if (!h5group.exists(name)) return false;
        hsize_t dim[2] = { len, 1 };
        // 创建数据集
    ...
    void FITKAbstractHDF5Adaptor::writeCharDataSet(H5::H5Object & h5group, std::string name, char* &data, int len)
    {
        hsize_t dim[2] = { len, 1 };
        // 创建数据集
        H5::DataSpace dataSpace(2, dim);
        H5::DataSet   dataSet = h5group.createDataSet(name, H5::PredType::NATIVE_CHAR, dataSpace);
